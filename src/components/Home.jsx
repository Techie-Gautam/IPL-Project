...
const Home = () => {
  ...
  const filterData = data.matchDetails.filter(
  ...
  const fetchMatchData = async () => {
    ...
    const options = {
      method: "GET",
      headers: {
        "x-rapidapi-key": "ddde8acfe2mshdb57456c8f496ddp154dfejsn9876db9f1627",
    ...
    try {
      ...
      const filterData = result.matchDetails.filter(
        (singleMatch) => singleMatch["matchDetailsMap"]
      );
  ...
  let imageUrl = "https://res.cloudinary.com/digkgdovw/image/upload/v1715267650/iplTeamLogo"
  ...
  return (
    <div className=" grid grid-cols-2 border border-t-0 border-l-0 overflow-hidden">
      {loading ? (
        <h1>Loading...</h1>
      ) : (
        matchesData?.map(({ matchDetailsMap: { match, key: date } }) =>
          match.map(({ matchInfo: { team1, team2, status, matchId,  matchDesc, state, matchFormat}, matchScore}, index) => (
            <Link key={index} to={`/matchDetail/${matchId}`}>
              {/* <h1>
                {team1.teamSName} vs {team2.teamSName} {key} {status}
              </h1> */}
              <div className="px-4 gap-3 border min-h-40 flex flex-col justify-center">
                ...
                <div>
                  <div className="flex justify-between mb-2">
                    ...
                    <img className="w-6" src={`${imageUrl}/${team1.imageId}`} alt="logo" />
                    <p>{team1.teamSName}</p>
                    </div>
                    {matchScore?.team1Score ? <p>{matchScore?.team1Score?.inngs1?.runs}/{matchScore?.team1Score?.inngs1?.wickets} ({matchScore?.team1Score?.inngs1?.overs})</p> : "NA"}
                  </div>
                  <div className="flex justify-between">
                    <div className="flex items-center gap-3">
                    <img className="w-6" src={`${imageUrl}/${team2.imageId}`} alt="logo" />
                    <p>{team2.teamSName}</p>
                    </div>
                    {matchScore?.team2Score ? <p>{matchScore?.team2Score?.inngs1?.runs}/{matchScore?.team2Score?.inngs1?.wickets} ({matchScore?.team2Score?.inngs1?.overs})</p> : "NA"}
                  </div>
                </div>
                <p>{status}</p>
              </div>
